          ...
          <table className="w-full min-w-[850px] text-left">
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} className="p-12 text-center font-black">Đang tải...</td></tr> : tab === 'periods' ? periods.map((item) => (
                <tr key={item.period_id} className="border-b-2 border-manga-ink"><td className="px-6 py-5 font-black">{item.name}</td><td className="px-5 py-5 font-semibold">{item.start_date} - {item.end_date}</td><td className="px-5 py-5">{item.period_type || 'N/A'}</td><td className="px-5 py-5"><AdminStatusBadge status={item.status || 'pending'} /></td><td className="px-5 py-5"><div className="flex justify-end gap-2"><button onClick={() => openEdit(item)} className={`${iconClass} bg-white text-manga-ink hover:bg-gray-100`} title="Chỉnh sửa"><Edit3 /></button><button onClick={() => remove('periods', item.period_id)} className={`${iconClass} bg-manga-red text-white hover:bg-red-600`} title="Xóa"><Trash2 /></button></div></td></tr>
              )) : tab === 'series' ? seriesRankings.map((item) => (
                <tr key={item.series_ranking_id} className="border-b-2 border-manga-ink"><td className="px-6 py-5 text-2xl font-black">#{item.rank_position || '-'}</td><td className="px-5 py-5 font-black">{item.series?.title || item.series_id}</td><td className="px-5 py-5">{item.period?.name || item.period_id}</td><td className="px-5 py-5 font-black">{item.score ?? 0}</td><td className="px-5 py-5">{item.total_vote ?? 0}</td><td className="px-5 py-5 text-right"><button onClick={() => remove('series', item.series_ranking_id)} className={`${iconClass} ml-auto bg-manga-red text-white hover:bg-red-600`} title="Xóa"><Trash2 /></button></td></tr>
              )) : chapterRankings.map((item) => (
                <tr key={item.chapter_ranking_id} className="border-b-2 border-manga-ink"><td className="px-6 py-5 text-2xl font-black">#{item.rank_position || '-'}</td><td className="px-5 py-5 font-black">{item.series?.title} / {item.chapter?.title || `Chương ${item.chapter?.chapter_number}`}</td><td className="px-5 py-5">{item.period?.name || item.period_id}</td><td className="px-5 py-5 font-black">{item.score ?? 0}</td><td className="px-5 py-5">{item.total_vote ?? 0}</td><td className="px-5 py-5 text-right"><button onClick={() => remove('chapters', item.chapter_ranking_id)} className={`${iconClass} ml-auto bg-manga-red text-white hover:bg-red-600`} title="Xóa"><Trash2 /></button></td></tr>
              ))}
            </tbody>
          </table>
        </div>
