...
export const adminPageTasksService = {
  list: (params?: { status?: PageTaskStatus; page?: number; limit?: number; sort?: string; order?: 'asc' | 'desc' }) =>
    adminList<PageTask>('/api/page-tasks', params),
  getById: (taskId: string) => adminGet<PageTask>(`/api/page-tasks/${taskId}`),
  ...
  update: (taskId: string, body: Partial<PageTask>) =>
    adminPatch<PageTask>(`/api/page-tasks/${taskId}`, body),
  updateStatus: (taskId: string, status: PageTaskStatus) =>
    adminPatch<PageTask>(`/api/page-tasks/${taskId}/status`, { status }),
  delete: (taskId: string) => adminDelete(`/api/page-tasks/${taskId}`),
}
