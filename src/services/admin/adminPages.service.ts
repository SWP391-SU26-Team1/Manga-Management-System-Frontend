...
export const adminPagesService = {
  list: (params?: { status?: PageStatus; page?: number; limit?: number; sort?: string; order?: 'asc' | 'desc' }) =>
    adminList<Page>('/api/pages', params),
  getById: (pageId: string) => adminGet<Page>(`/api/pages/${pageId}`),
  ...
  updateStatus: (pageId: string, status: PageStatus) =>
    adminPatch<Page>(`/api/pages/${pageId}/status`, { status }),
  retryOcr: (pageId: string) => adminPost<Page>(`/api/pages/${pageId}/retry-ocr`),
  delete: (pageId: string) => adminDelete(`/api/pages/${pageId}`),
}
