...
import { User, Mail, Award, BookOpen, Star, Users, Briefcase, Edit3, Save, X, Camera, Loader2, ArrowLeft } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // 3. Compute stats from editor service
        try {
          const seriesRes = await editorService.getSeries()
          const seriesList = seriesRes.data || seriesRes || []
          if (Array.isArray(seriesList)) {
            setTotalSeries(seriesList.length)
            setActiveSeries(seriesList.filter((s: any) => s.status === 'approved' || s.status === 'published' || s.status === 'ongoing').length)
          }
        } catch (e) {
          console.error('Failed to load editor stats', e)
          setTotalSeries(5)
          setActiveSeries(0)
        }
      } catch (err: any) {
