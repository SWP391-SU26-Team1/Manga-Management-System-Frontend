...
import { mangakaStore, RiskAlert } from '@/data/mangakaMockData'
...
export default function RiskAlertsPage() {
  ...
  const stats = [
    ...
    { label: "Chưa đọc", value: alerts.filter(a => !a.isRead).length, icon: Info, color: "text-orange-500" },
  ]
  ...
  const handleMarkAsRead = (id: string, e?: React.MouseEvent) => {
    ...
    setAlerts(updated);
    mangakaStore.markRiskAlertRead(id);
  }

  const handleCreateProposal = (seriesId: string) => {
    navigate(`/dashboard/mangaka/recovery-proposal?seriesId=${seriesId}`);
  }
          ...
          {alerts.map(alert => (
            <div 
              ...
              className={`p-6 flex items-start sm:items-center justify-between flex-col sm:flex-row gap-4 transition-colors cursor-pointer ${
                !alert.isRead ? 'bg-red-50/40 hover:bg-red-50/60' : 'hover:bg-gray-50'
              }`}
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    {alert.seriesId}
                    {!alert.isRead && <span className="w-2 h-2 rounded-full bg-red-500 inline-block"></span>}
                  </h3>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white border-4 border-manga-ink manga-shadow max-w-2xl w-full flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 border-t-4 border-manga-ink bg-gray-50 flex justify-between items-center shrink-0">
              ...
              <button 
                onClick={() => handleCreateProposal(selectedAlert.seriesId)}
