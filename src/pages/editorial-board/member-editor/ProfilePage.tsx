...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // Fallback to Board mock data if not found
      loadedProfile = MOCK_USERS.find(u => u.role === 'BOARD')
    }
  ...
  const userInitials = profile.fullName === 'Minamoto Shizuka' 
    ? 'MS' 
      ...
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-sm font-bold text-gray-500">
            Quản lý thông tin công khai và các chỉ số hoạt động của ủy viên hội đồng
          </p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-white border-4 border-manga-ink shadow-[8px_8px_0px_rgba(0,0,0,1)] p-6 flex flex-col items-center text-center">
            ...
            <div className="inline-block px-3 py-1 bg-manga-red text-white font-bold uppercase text-xs border-2 border-manga-ink mb-4">
              {profile.role === 'BOARD' ? 'MEMBER EDITOR' : profile.role}
            </div>
          ...
          <div className="bg-white border-4 border-manga-ink p-6 shadow-[4px_4px_0px_rgba(15,15,15,1)]">
            ...
            <p className="text-gray-700 font-medium text-sm leading-relaxed">
              {profile.bio || 'Ủy viên ban biên tập thẩm định dự án mới.'}
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white border-4 border-manga-ink p-4 flex flex-col items-center text-center shadow-[4px_4px_0px_rgba(15,15,15,1)]">
              ...
              <span className="text-3xl font-black font-manga">{profile.stats?.rating?.toFixed(1) || '4.9'}</span>
          ...
          <div className="bg-white border-4 border-manga-ink p-6 shadow-[6px_6px_0px_rgba(15,15,15,1)]">
            ...
            <div className="flex flex-wrap gap-3">
              {boardSkills.map((item, idx) => (
                <div 
                  key={idx} 
                  className="px-4 py-2 bg-gray-50 border-2 border-manga-ink font-bold text-sm uppercase shadow-[2px_2px_0px_rgba(15,15,15,1)] hover:bg-manga-red hover:text-white transition-colors cursor-default"
                >
                  {item}
                </div>
              ))}
